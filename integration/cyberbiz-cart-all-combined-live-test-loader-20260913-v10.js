/*
 * eYeFANS all-combined-products live-test entry — 2026-09-13 v10.
 *
 ...
 *
 * Product pages load the v4.9 core only with
 * `?eyefans_all_combined_live_test=1`. Checkout pages continue loading it only
 * while this browser has non-empty isolated test records.
 * An additional exact `eyefans_anniversary_preview=1` parent query is forwarded
 * to the customizer only for this isolated test runtime.
 */
(function eyefansCyberbizCartAllCombinedLiveTestEntryV10() {
  "use strict";

  const STOREFRONT_ORIGIN = "https://www.eyefans.com.tw";
  const TEST_QUERY_KEY = "eyefans_all_combined_live_test";
  const TEST_QUERY_VALUE = "1";
  const ANNIVERSARY_PREVIEW_QUERY_KEY = "eyefans_anniversary_preview";
  const TEST_STORAGE_KEY = "eyefansCustomCartDesignsAllCombinedDevV1";
  const WRAPPER_FLAG = "__eyefansCartAllCombinedLiveTestEntryV10Active";
  const CONFLICTING_LOADER_FLAGS = Object.freeze([
    ...
    "__eyefansCartAllCombinedLiveTestEntryV7Active",
    "__eyefansCartAllCombinedLiveTestEntryV8Active",
    "__eyefansCartAllCombinedLiveTestEntryV9Active",
    "__eyefansCartProductionLoaderActive"
  ]);
  const BLOCKING_LOADER_FLAGS = Object.freeze([
    ...CONFLICTING_LOADER_FLAGS,
  ...
  if (
    ...
    || window[WRAPPER_FLAG]
    || BLOCKING_LOADER_FLAGS.some(flag => window[flag])
  ) return;
  ...
  const explicitAnniversaryPreview = explicitProductTest
  ...
  coreUrl.searchParams.set("eyefans_all_combined_development", "1");
  if (explicitAnniversaryPreview) {
    coreUrl.searchParams.set(ANNIVERSARY_PREVIEW_QUERY_KEY, "1");
  }

  const script = document.createElement("script");
  script.src = coreUrl.href;
  script.integrity = "sha384-KQ1IkO0yaaVICtRLSyBp8D58txz3JMpklO0UlHOIA3aCDR30mr4qsqHYdlU9bt+S";
  ...
  function guardCheckoutAfterCoreLoadFailure() {
    ...
    window.__eyefansCartAllCombinedCoreLoadFailed = true;
    ...
    if (typeof window.jQuery === "function") {
      window.jQuery(document).on(
        "checkout_cart:checkout.eyefansAllCombinedCoreLoadFailure",
        event => {
          event?.preventDefault?.();
          event?.stopImmediatePropagation?.();
          disableCheckoutButtons();
