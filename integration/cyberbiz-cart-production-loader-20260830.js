/*
 * eYeFANS CYBERBIZ integration — cart-identity guarded bridge (2026-08-30).
 *
...
(function eyefansCyberbizCartProductionLoader() {
  ...
  async function cartJsonState(variantId, signal) {
    ...
    return { variantQuantity, itemCount: totalItemCount };
  }
  ...
  async function resolveSessionCartToken(signal, allowEmptyCart = false) {
    ...
    try {
      ...
      if (allowEmptyCart && /^\/cart\/?$/.test(destination.pathname)) return null;
      throw new Error("CART_IDENTITY_UNVERIFIED");
    } finally {
  ...
  async function addVariantToCart(context, variantId, size, options = {}) {
    ...
    try {
      ...
      let beforeCartToken;
      try {
        beforeCartToken = await resolveSessionCartToken(controller.signal, cartResult.value.itemCount === 0);
      } catch (error) {
        if (controller.signal.aborted) throw new Error("CART_REQUEST_TIMEOUT");
        throw new Error("CART_PREFLIGHT_FAILED");
      }
      ...
      const preflightResult = options.onPreflight?.(beforeQuantity, { canPost, cartToken: beforeCartToken });
      ...
      let afterQuantity;
      try {
        afterQuantity = (await cartJsonState(variantId, controller.signal)).variantQuantity;
      } catch (error) {
        if (error?.name === "AbortError" || controller.signal.aborted) {
          throw new Error("CART_REQUEST_TIMEOUT");
        }
    ...
    async function processRequest(request, targetWindow) {
      ...
      try {
        addResult = await addVariantToCart(context, variantId, request.selection.size, {
          onPreflight(beforeQuantity, { canPost, cartToken: sessionCartToken }) {
            ...
            if (cartCandidates.some(item => item.status === "pending")) throw new Error("CART_REQUEST_TIMEOUT");
            ...
            zeroBaselineRecordIds = beforeQuantity === 0
      ...
      try {
        ...
        const reconciledRecords = records.flatMap(item => {
          ...
          if (!zeroBaselineRecordIds?.has(item.requestId) || item.status !== "active" || item.variantId !== variantId) {
            return [item];
          }
