/*
 * eYeFANS CYBERBIZ integration — ALL COMBINED PRODUCTS CANDIDATE V4.10.
 *
 * UNPUBLISHED PRODUCTION CANDIDATE V4.10 / DO NOT INSTALL ON A PUBLISHED THEME.
 *
 * Color, engraving, and UV each keep all three lenses in one CYBERBIZ product,
 * with an exact SIZE × LENS variant for every combination. Three target
 * products and all 36 variants are pinned below. This candidate may be used only as a
 * replacement loader in an unpublished theme for QA. Product JSON, option
 * axes, inventory, and exact target identity are checked before POST.
 ...
 * The anniversary-price preview is display-only and is forwarded to the iframe
 * only by the isolated development runtime after two exact parent query gates.
 * On mobile, the native sticky cart controls are replaced by one safe
 * "開始客製設計" action that scrolls to the customizer without adding a product.
 * CYBERBIZ cart payloads may expose a variant as `variant_id_int`, a numeric
 * `variant_id`, or a cart-key-shaped `variant_id` such as `123_normal_`.
 * All three verified formats resolve to the same numeric variant identity.
 * UV submissions must include icons and/or a name; the retired no-print UV
 * choice is rejected even if a stale iframe attempts to submit it.
 * Cosmetic checkout rerenders are ignored unless the cart's rendered quantity
 * signature changed or a shopper explicitly started a cart mutation.
 * Guarded checkout pages also receive the standard dataLayer-backed `gtag`
 * queue when CYBERBIZ calls analytics without loading Google Analytics first.
 *
 ...
 *
 * This browser-side bridge is a production fallback for CYBERBIZ's standard
 ...
 */
(function eyefansCyberbizCartAllCombinedProductionCandidateV4_11Loader() {
  "use strict";

  const TEST_QUERY_KEYS = Object.freeze(["eyefans_cart_test", "eyefans_cart_live_test"]);
  const VERSIONED_TEST_QUERY_KEYS = Object.freeze([
    "eyefans_uv_combined_live_test",
    "eyefans_all_combined_live_test"
  ]);
  const STOREFRONT_ORIGIN = "https://www.eyefans.com.tw";
  const ALL_COMBINED_TEST_QUERY_KEY = "eyefans_all_combined_live_test";
  ...
  const CUSTOMIZER_UI_VERSION = "20260910-no-none";
  ...
  const loaderRuntime = (() => {
    try {
      ...
      return Object.freeze({
        development: loaderUrl.searchParams.get("eyefans_all_combined_development") === "1",
        anniversaryPreview: loaderUrl.searchParams.get(ANNIVERSARY_PREVIEW_QUERY_KEY) === "1"
      });
    } catch (error) {
      return Object.freeze({ development: false, anniversaryPreview: false });
    }
  })();
  const developmentRuntime = loaderRuntime.development;
    ...
    "cls-cus-mix-sun-rd": Object.freeze({
      ...
      targetsByLens: Object.freeze({
        ...
        "blue-tea": Object.freeze({
          ...
          optionValue: "抗藍光",
          variantsBySize: Object.freeze({
            XS: "87870151", S: "87870152", M: "87870153", L: "87870154"
          })
        }),
        polarized: Object.freeze({
          ...
          optionValue: "偏光",
          variantsBySize: Object.freeze({
            XS: "87870155", S: "87870157", M: "87870158", L: "87870159"
          })
        })
      })
    }),
    "cls-cus-mix-laser-sun-rd": Object.freeze({
      ...
      targetsByLens: Object.freeze({
        ...
        "blue-tea": Object.freeze({
          ...
          optionValue: "抗藍光鏡片",
          variantsBySize: Object.freeze({
            XS: "87856080", S: "87856081", M: "87856082", L: "87856083"
          })
        }),
        polarized: Object.freeze({
          handle: "cls-cus-mix-laser-sun-rd",
          ...
          optionValue: "偏光鏡片",
          variantsBySize: Object.freeze({
            XS: "87856084", S: "87856085", M: "87856086", L: "87856087"
          })
        })
      })
    }),
    "cls-cus-mix-uv-sun-rd": Object.freeze({
      ...
      targetsByLens: Object.freeze({
        gray: Object.freeze({
          ...
          optionValue: "灰片",
          variantsBySize: Object.freeze({
            XS: "87452778", S: "87817315", M: "87817316", L: "87852179"
          })
        }),
        "blue-tea": Object.freeze({
          handle: "cls-cus-mix-uv-sun-rd",
          ...
          optionValue: "抗藍光",
          variantsBySize: Object.freeze({
            XS: "87852180", S: "87852181", M: "87852182", L: "87852183"
          })
        }),
        polarized: Object.freeze({
          handle: "cls-cus-mix-uv-sun-rd",
          ...
          optionValue: "偏光",
          variantsBySize: Object.freeze({
            XS: "87852184", S: "87852185", M: "87852186", L: "87852188"
          })
        })
      })
    })
  ...
  // These retired separate-product variants must never be submitted again.
  // They remain in
  // the checkout guard only so a stale pre-migration cart fails closed and
  // asks the customer to remove/re-add the item. Old 87452778 is not repeated
  // here because CYBERBIZ reassigned that id from gray M to the current gray XS;
  // storedRecordMatchesTarget rejects its former M record semantics.
  const RETIRED_CUSTOM_VARIANT_IDS = new Set([
  ...
  const CHECKOUT_GUARDED_VARIANT_IDS = new Set([
    ...ALL_CUSTOM_VARIANT_IDS,
    ...RETIRED_CUSTOM_VARIANT_IDS
  ]);
  ...
  // The versioned wrapper loads this same core in development mode. A direct
  // production copy must stay inert on that wrapper's explicit test URL.
  if (
    !developmentRuntime
    && VERSIONED_TEST_QUERY_KEYS.some(key => pageUrl.searchParams.get(key) === "1")
  ) return;
  // A query typed on a production URL is never sufficient. The versioned test
  // entry must attest the same exact preview flag on this core's own URL.
  const anniversaryPreviewEnabled = developmentRuntime
    && loaderRuntime.anniversaryPreview
    && pageUrl.searchParams.get(ALL_COMBINED_TEST_QUERY_KEY) === "1"
    && pageUrl.searchParams.get(ANNIVERSARY_PREVIEW_QUERY_KEY) === "1";
  ...
  if (guardedCheckout) ensureCheckoutAnalyticsCompatibility();
  ...
  function ensureCheckoutAnalyticsCompatibility() {
    if (typeof window.gtag === "function") return;
    const dataLayer = Array.isArray(window.dataLayer) ? window.dataLayer : [];
    window.dataLayer = dataLayer;
    // This is the same queueing contract used by Google's standard gtag.js
    // bootstrap. If analytics loads later it consumes the queued arguments;
    // meanwhile CYBERBIZ checkout analytics cannot throw and unmount the form.
  ...
  // Live CYBERBIZ cart JSON includes earned gifts in items, but excludes
  // them from item_count and total_quantity. Keep every item for identity
  // reconciliation; allow exclusion from summary counters only for an
  // explicit gift type with a matching platform gift variant identifier.
  function cartSummaryCountsMatch(payload, totalQuantity) {
    const gifts = payload.items.filter(item => item.type === "gift"
      && String(item.variant_id) === `${cartItemVariantId(item)}_gift_`);
    const giftQuantity = gifts.reduce((sum, item) => sum + Number(item.quantity), 0);
    return [[payload.items.length, totalQuantity],
      [payload.items.length - gifts.length, totalQuantity - giftQuantity]]
      .some(([rows, units]) => {
        const count = Number(payload.item_count);
        if (count !== rows && count !== units) return false;
        if (!Object.prototype.hasOwnProperty.call(payload, "total_quantity")) return true;
        const reported = Number(payload.total_quantity);
        return Number.isInteger(reported) && reported >= 0 && reported === units;
      });
  }
  ...
  function currentCartToken() {
    // Mobile CVS selection returns here before Checkout v3 replaces the URL.
    // A deferred loader may run on either side of that rewrite.
    const match = pageUrl.pathname.match(/^\/carts\/([A-Za-z0-9_-]+)(?:\/redirect_cvs)?\/?$/i);
  ...
  function checkoutContainsCustomVariant() {
    ...
    return lineItems.some(item => CHECKOUT_GUARDED_VARIANT_IDS.has(String(item?.variant_id || "")));
  }
  ...
  function validStoredReceipt(record) {
    ...
    if (receipt.verifiedByCustomer === true) {
      return record.cartToken !== null
        && receipt.cartToken === record.cartToken
        && String(receipt.variantId) === String(record.variantId)
        && receipt.cartItemId === `${record.variantId}_normal_`
        && receipt.quantity === 1
        && receipt.verifiedByCartDelta === false
        && Number.isFinite(receipt.verifiedAt) && receipt.verifiedAt >= record.createdAt;
    }
  ...
  function validateModeSelection(selection, expectedMode) {
    ...
    if (selection.printMode === "none") throw new Error("INVALID_UV_MODE");
  ...
  function publicErrorMessage(error) {
    const messages = {
      ...
      INVALID_UV_MODE: "UV 彩印商品請選擇圖案或名字內容。",
      ...
      CART_PREFLIGHT_FAILED: "目前無法讀取 CYBERBIZ 購物車，因此尚未送出商品。請稍後再試。（P00）",
      CART_PREFLIGHT_READ_FAILED: "購物車資料暫時無法讀取，尚未送出商品。請稍後再試；若持續發生，請截圖聯絡客服。（P01）",
      CART_PREFLIGHT_DATA_INVALID: "購物車回傳資料無法核對，尚未送出商品。請截圖聯絡客服，並保留目前瀏覽器資料。（P02）",
      CART_PREFLIGHT_SESSION_FAILED: "購物車連線暫時無法確認，尚未送出商品。請稍後再試；若持續發生，請截圖聯絡客服。（P03）",
      CART_PREFLIGHT_SESSION_INVALID: "購物車識別資料無法確認，尚未送出商品。請截圖聯絡客服，並保留目前瀏覽器資料。（P04）",
    ...
    const message = messages[error?.message] || "客製資料不完整或目前無法加入購物車，請重新確認。";
    const detail = error?.identityDetail;
    return error?.message === "CART_PREFLIGHT_SESSION_INVALID"
      && ["URL", "ORIGIN", "HOME", "ROUTE", "CHANGED"].includes(detail)
      ? message.replace("（P04）", `（P04-${detail}）`) : message;
  }
  ...
  async function parseCartResponse(response, requestedVariantId) {
    ...
    const returnedVariantId = cartItemVariantId(payload);
  ...
  async function verifyTargetProductVariant(target, variantId, size, signal) {
    ...
    if (typeof target.optionValue === "string") {
      if (
        !Array.isArray(payload.options)
        || payload.options.length !== 2
        || payload.options[0] !== "尺寸"
        || payload.options[1] !== "鏡片"
      ) {
        throw new Error("PRODUCT_VARIANT_MISMATCH");
      }
      matchingVariants = payload.variants.filter(variant => (
        isPlainObject(variant)
        && variant.option1 === size
        && variant.option2 === target.optionValue
    ...
    if (matchingVariants.length !== 1) throw new Error("PRODUCT_VARIANT_MISMATCH");
  ...
  async function cartJsonState(variantId, signal) {
    ...
    try {
      ...
      throw Object.assign(new Error("CART_STATE_UNAVAILABLE"), { retryable: true });
    }
    const contentType = response.headers?.get?.("content-type") || "";
    if (!response.ok) throw Object.assign(new Error("CART_STATE_UNAVAILABLE"), {
    ...
    if (!cartSummaryCountsMatch(payload, totalQuantity)) {
      throw new Error("CART_STATE_INVALID");
    }
    ...
    if (hasTotalQuantity && (
      ...
      || !cartSummaryCountsMatch(payload, totalQuantity)
    )) throw new Error("CART_STATE_INVALID");
  ...
  async function resolveSessionCartToken(signal, allowEmptyCart = false) {
    ...
    let response;
    try {
      response = await window.fetch(new URL(CART_URL, STOREFRONT_ORIGIN).href, {
        method: "GET",
        mode: "same-origin",
        credentials: "same-origin",
      ...
      if (error?.name === "AbortError" || signal?.aborted) throw error;
      throw Object.assign(new Error("CART_IDENTITY_UNAVAILABLE"), { retryable: true });
    }
    try {
      if (!response.ok) throw Object.assign(new Error("CART_IDENTITY_UNAVAILABLE"), {
        retryable: response.status === 408 || response.status === 425 || response.status >= 500
      });
      const identityError = detail => Object.assign(new Error("CART_IDENTITY_UNVERIFIED"), { identityDetail: detail });
      if (typeof response.url !== "string" || !response.url) throw identityError("URL");
      let destination;
      try { destination = new URL(response.url); } catch (error) { throw identityError("URL"); }
      if (destination.origin !== STOREFRONT_ORIGIN || destination.username || destination.password) {
        throw identityError("ORIGIN");
      }
      ...
      // Compare the final same-origin URL, not Response.redirected metadata.
      // Null is only a preflight empty-cart state, NEVER a verified identity.
      const emptyHomeDestination = destination.pathname === "/"
        && destination.search === ""
        && destination.hash === "";
      // A signed-in empty cart may redirect to the member index in LINE.
      // This is only an empty preflight state, never a cart identity.
      const emptyMemberDestination = response.redirected === true
        && destination.pathname === "/account/index"
        && destination.search === ""
        && destination.hash === "";
      if (allowEmptyCart && (/^\/cart\/?$/.test(destination.pathname) || emptyHomeDestination || emptyMemberDestination)) return null;
      throw identityError(destination.pathname === "/" ? "HOME" : "ROUTE");
    } finally {
  ...
  async function readCartPreflight(variantId, signal) {
    // Retry only transient GET failures, once, under the existing 15-second
    // request deadline. Re-read quantity AND identity together on each attempt.
    // Never retry malformed data, an untrusted redirect, or any POST.
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        const state = await cartJsonState(variantId, signal);
        phase = "session";
        const cartToken = await resolveSessionCartToken(signal, state.isEmpty);
        if (cartToken === null) {
          // The session route issued no identity: confirm emptiness again
          // after navigation. Concurrent cart changes must fail closed.
          phase = "data";
          const confirmed = await cartJsonState(variantId, signal);
          if (!confirmed.isEmpty) {
            phase = "session";
            throw Object.assign(new Error("CART_IDENTITY_UNVERIFIED"), { identityDetail: "CHANGED" });
          }
        }
        if (signal.aborted) throw new Error("CART_REQUEST_TIMEOUT");
        return { ...state, cartToken };
      } catch (error) {
        if (error?.name === "AbortError" || signal.aborted) throw new Error("CART_REQUEST_TIMEOUT");
        if (attempt === 0 && error?.retryable === true) {
          await new Promise(resolve => window.setTimeout(resolve, 350));
          continue;
        }
        const code = phase === "data"
          ? (error?.message === "CART_STATE_UNAVAILABLE" ? "CART_PREFLIGHT_READ_FAILED" : "CART_PREFLIGHT_DATA_INVALID")
          : (error?.message === "CART_IDENTITY_UNAVAILABLE" ? "CART_PREFLIGHT_SESSION_FAILED" : "CART_PREFLIGHT_SESSION_INVALID");
        throw Object.assign(new Error(code), {
          identityDetail: ["URL", "ORIGIN", "HOME", "ROUTE", "CHANGED"].includes(error?.identityDetail)
            ? error.identityDetail : null
        });
      }
    }
  }
  ...
  async function addVariantToCart(target, variantId, size, options = {}) {
    ...
    try {
      ...
      const preflightResults = await Promise.allSettled([
        ...
        readCartPreflight(variantId, controller.signal)
      ]);
      ...
      const beforeCartToken = cartResult.value.cartToken;
      ...
      renewDeadline(); // Product/session preflight must not consume the POST budget.
      ...
      let afterQuantity;
      // An uncertain POST is never repeated. Give read-only verification its
      // own deadline, including when the POST response itself timed out.
      renewDeadline();
      try {
      ...
      if ([
        ...
        "CART_PREFLIGHT_READ_FAILED", "CART_PREFLIGHT_DATA_INVALID",
        "CART_PREFLIGHT_SESSION_FAILED", "CART_PREFLIGHT_SESSION_INVALID",
    ...
    return frames.find(frame => {
      ...
      try {
        ...
        return iframeUrl.origin === CUSTOMIZER_ORIGIN
          ...
          && (!requireCartGate || (
            iframeUrl.searchParams.get("cart") === "1"
            && iframeUrl.searchParams.get("ui") === CUSTOMIZER_UI_VERSION
          ));
      } catch (error) {
        return false;
      }
  ...
  function lockNativeProductPurchase(context) {
    ...
    const mobileStartSelector =
      `#product_info .product_button_mobile > button.addToCart_mobile.btn_to_cart_mobile[data-handle="${handle}"]`;
    const blockedControlSelector = [
      `#product_info .product_button > button.addToCart[data-id="${productId}"]`,
      `#product_info .product_button_mobile > button.addToCart_mobile.btn_quick_buy_mobile[data-handle="${handle}"]`,
    ...
    const controlSelector = `${mobileStartSelector}, ${blockedControlSelector}`;
    const markedControlSelector = "[data-eyefans-native-purchase]";
    const blockedSelector = "[data-eyefans-native-purchase='blocked']";
    const mobileStartLabel = "開始客製設計";
    const blockedLabel = "請使用下方模擬器加入購物車";
    ...
    function markControls() {
      const mobileStarts = Array.from(document.querySelectorAll?.(mobileStartSelector) || []);
      mobileStarts.forEach(control => {
        control.dataset.eyefansNativePurchase = "customizer";
        control.setAttribute?.("aria-disabled", "false");
        control.setAttribute?.("aria-label", mobileStartLabel);
        control.setAttribute?.("title", "前往下方客製模擬器開始設計");
        control.setAttribute?.("type", "button");
        if ("disabled" in control && control.disabled !== false) control.disabled = false;
        if (control.textContent !== mobileStartLabel) control.textContent = mobileStartLabel;
      });

      const blockedControls = Array.from(document.querySelectorAll?.(blockedControlSelector) || []);
      blockedControls.forEach(control => {
        ...
        if (control.tagName === "INPUT") {
          if (control.value !== blockedLabel) {
            control.value = blockedLabel;
          }
        } else if (control.textContent !== blockedLabel) {
          control.textContent = blockedLabel;
        }
      });
    }
    ...
    function blockNativeClick(event) {
      ...
      if (control.matches?.(mobileStartSelector)) {
        document.querySelector?.(CUSTOMIZER_IFRAME_SELECTOR)?.scrollIntoView?.({
          behavior: "smooth",
          block: "start"
        });
      }
    ...
    function blockNativeSubmit(event) {
      if (!event.target?.querySelector?.(markedControlSelector)) return;
      event.preventDefault?.();
      event.stopImmediatePropagation?.();
    }
    ...
    if (document.head && document.createElement && !document.getElementById?.("eyefans-native-purchase-lock-style")) {
      ...
      style.textContent = [
        ...
        `#product_info .product_button_mobile>${blockedSelector}{display:none!important}`,
        `#product_info .product_button_mobile>[data-eyefans-native-purchase='customizer']{flex:1 1 auto!important}`
      ].join("");
  ...
  function startProductBridge(context) {
    ...
    function enable() {
      ...
      const iframeUrl = new URL(frame.src, pageUrl.href);
      const currentPreviewValue = iframeUrl.searchParams.get(ANNIVERSARY_PREVIEW_QUERY_KEY);
      const previewNeedsUpdate = anniversaryPreviewEnabled
        ? currentPreviewValue !== "1"
        : currentPreviewValue !== null;
      const uiNeedsUpdate = iframeUrl.searchParams.get("ui") !== CUSTOMIZER_UI_VERSION;
      if (iframeUrl.searchParams.get("cart") !== "1" || previewNeedsUpdate || uiNeedsUpdate) {
        iframeUrl.searchParams.set("cart", "1");
        iframeUrl.searchParams.set("ui", CUSTOMIZER_UI_VERSION);
        if (anniversaryPreviewEnabled) {
          iframeUrl.searchParams.set(ANNIVERSARY_PREVIEW_QUERY_KEY, "1");
        } else {
          iframeUrl.searchParams.delete(ANNIVERSARY_PREVIEW_QUERY_KEY);
        }
  ...
  function startCartNoteSync(activeCartToken) {
    ...
    let lastVerifiedRecordsSnapshot = null;
    function currentRecordsSnapshot() {
      // Compare raw storage as well as parsed records: malformed/removed data
      // must invalidate cached manufacturing notes, never silently match [].
      try { return window.localStorage.getItem(STORAGE_KEY) || "[]"; }
      catch (_) { return null; }
    }
    function verifiedRecordsStillCurrent() {
      return lastVerifiedRecordsSnapshot !== null
        && currentRecordsSnapshot() === lastVerifiedRecordsSnapshot;
    }
    ...
    let lastVerifiedDomSignature = null;
    let cartMutationPending = false;
    let mutationRetryAvailable = false;
    let recoveryConsent = null;
    ...
    function renderPanel(noteBlock, errorMessage) {
      ...
      // Checkout mounts asynchronously and replaces its note section after CVS
      // return. Relocate an early panel instead of leaving it below the footer.
      const note = document.querySelector('textarea[name="order[note]"]');
      const parent = note?.parentElement || document.querySelector(CHECKOUT_BUTTON_SELECTOR)?.parentElement || document.body;
      if (panel.parentElement !== parent) parent.insertBefore(panel, note || null);
    ...
    function currentDomCartSnapshot() {
      const allRows = Array.from(document.querySelectorAll(CART_LINE_ITEM_SELECTOR))
        // CYBERBIZ renders an unearned gift preview as a line-item too.
        // Exclude only this explicit preview marker; earned gifts and other
        // non-adjustable rows must still match the authoritative cart JSON.
        .filter(row => !(
          row?.matches?.("tr.line-item.extra.next-gift, div.line-item.extra.next-gift")
          && !row?.querySelector?.('[data-testid="quantity-input"]')
        ));
      ...
      let earnedGiftRows = 0;
      for (const row of rows) {
        const quantityInput = row?.querySelector?.('[data-testid="quantity-input"]');
        if (!quantityInput && row?.matches?.("tr.line-item.extra.gift, div.line-item.extra.gift")) {
          earnedGiftRows += 1;
        }
        if (!quantityInput) {
      ...
      return {
        ...
        signature: `${rows.length}:${quantities.join(",")}:g${earnedGiftRows}`
      };
    }
    ...
    function parseAuthoritativeCartPayload(payload) {
      ...
      for (const item of payload.items) {
        ...
        if (!CHECKOUT_GUARDED_VARIANT_IDS.has(variantId)) continue;
        quantities.set(variantId, (quantities.get(variantId) || 0) + quantity);
      }

      const rowCount = payload.items.length;
      const itemCount = Number(payload.item_count);
      if (!cartSummaryCountsMatch(payload, totalQuantity)) return null;
      if (Object.prototype.hasOwnProperty.call(payload, "total_quantity")) {
        const reportedTotalQuantity = Number(payload.total_quantity);
        if (!Number.isInteger(reportedTotalQuantity) || !cartSummaryCountsMatch(payload, totalQuantity)) return null;
      }
    ...
    function reconcile(cartState) {
      try {
        ...
        const recordsSnapshot = currentRecordsSnapshot();
        ...
        const candidates = [...boundCurrentRecords, ...eligibleUnboundRecords];
        // Preserve history, but do not count removed/other variants as current
        // manufacturing candidates. A recent unknown add may still arrive:
        // retain a bounded safety hold rather than treating it as absent.
        const unresolvedRecentAdd = candidates.some(record => record.status === "pending"
          && !quantities.has(record.variantId) && Date.now() - record.createdAt <= RETRY_GUARD_MS);
        if (unresolvedRecentAdd) {
          return blockUnverifiedDesigns("有一筆最近送出的客製商品尚未確認是否加入，暫時保留設計並等待核對。請勿重複加入；稍後重新整理購物車。若超過五分鐘仍無法恢復，請聯絡客服。");
        }
        let records = candidates.filter(record => quantities.has(record.variantId));
        ...
        if (quantities.size === 0) {
          ...
          // A transient server/DOM snapshot must never destroy original designs.
          // Only current, identity-verified records are used in the submitted note.
          lastVerifiedNoteValue = manualNote;
          if (recordsSnapshot === null || recordsSnapshot !== currentRecordsSnapshot()) return false;
          lastVerifiedRecordsSnapshot = recordsSnapshot;
        ...
        if (records.some(record => record.status === "pending")) {
          // Old pending records have no trustworthy cart identity. Never bind
          // them from quantity alone: show every candidate and require the
          // customer's explicit manufacturing confirmation, then fresh reads.
          const exact = records.every(record => quantities.has(record.variantId)
            && (record.status === "pending" || record.cartToken === activeCartToken))
            && Array.from(quantities).every(([id, qty]) => records.filter(r => r.variantId === id).length === qty);
          const snapshot = JSON.stringify(records);
          if (exact && recoveryConsent?.signature === cartState.signature
            && recoveryConsent.records === snapshot) {
            const acceptedIds = new Set(records.filter(r => r.status === "pending").map(r => r.requestId));
            writeRecords(allRecords.map(record => acceptedIds.has(record.requestId) ? {
              ...record, status: "active", cartToken: activeCartToken,
              receipt: {variantId: String(record.variantId), cartItemId: `${record.variantId}_normal_`,
                quantity: 1, cartToken: activeCartToken, verifiedByCustomer: true,
                verifiedByCartDelta: false, verifiedAt: Date.now()}
            } : record));
            recoveryConsent = null;
            return reconcile(cartState);
          }
          recoveryConsent = null;
          blockUnverifiedDesigns("加入確認曾中斷，原始設計仍保留。請逐筆核對下方尺寸、配色、圖案及文字；尚未確認前不會送出訂單。\n\n"
            + buildNoteBlock(records) + (exact ? "\n\n請確認以上每一筆都是本次要製作的設計。" : "\n\n商品數量或設計無法一一對應，請將此畫面交給客服處理，不要重複加入。"));
          if (exact) {
            const button = document.createElement("button");
            button.type = "button";
            button.id = "eyefans-confirm-pending-designs";
            button.textContent = "我已逐筆核對，確認以上設計用於本次購物車";
            button.style.cssText = "padding:12px;margin-top:12px;white-space:normal;max-width:100%";
            button.onclick = () => {
              button.disabled = true;
              recoveryConsent = {signature: cartState.signature, records: snapshot};
              startSyncRun(false, true).then(synced => {
                if (!synced) recoveryConsent = null;
              }).catch(() => { recoveryConsent = null; });
            };
            document.getElementById("eyefans-cart-design-summary")?.append(button);
          }
          return true;
        }
        ...
        // Retain records absent from this snapshot for return/recovery. Matching
        // remains exact by cart token, variant and quantity; never guess a design.
    ...
    async function settleAndSync(expectedMutationVersion, expectedSyncRunVersion, requireChangedCart, recoverUnchanged = false) {
      let previousDomSignature = null;
      let stableDomSamples = 0;
      let previousAuthoritativeSignature = null;
      let stableAuthoritativeSamples = 0;
      ...
      for (const delay of CART_SYNC_RETRY_DELAYS_MS) {
        ...
        // CYBERBIZ may render earned order gifts separately from /cart.json
        // purchasable items. Accept either the full representation or the exact
        // one with explicit, non-adjustable .extra.gift rows omitted. Never
        // exclude ordinary/add-buy/unknown rows or alter guarded quantities.
        const matchesRows = omittedGifts => {
          const missing = domSnapshot.missingQuantityCount - omittedGifts;
          return cartState.rowCount === domSnapshot.rowCount - omittedGifts
            && (missing === 0
              ? cartState.totalQuantity === domSnapshot.totalQuantity
              : cartState.totalQuantity >= domSnapshot.totalQuantity + missing);
        };
        if (!matchesRows(0) && !(domSnapshot.earnedGiftRows > 0 && matchesRows(domSnapshot.earnedGiftRows))) {
          previousAuthoritativeSignature = null;
          stableAuthoritativeSamples = 0;
          continue;
        }
        if (previousAuthoritativeSignature === cartState.signature) stableAuthoritativeSamples += 1;
        else {
          previousAuthoritativeSignature = cartState.signature;
          stableAuthoritativeSamples = 1;
        }
        ...
        if (
          requireChangedCart
          && !(recoverUnchanged && stableAuthoritativeSamples >= 2)
          && (
        ...
        // Confirmation cannot reuse the earlier cart response: require two
        // stable authoritative reads and the normal DOM/mutation guards.
        if (recoveryConsent && stableAuthoritativeSamples < 2) continue;
        ...
        lastVerifiedDomSignature = domSnapshot.signature;
        if (requireChangedCart) cartMutationPending = false;
        mutationRetryAvailable = false;
        return true;
      }
      ...
      if (
        ...
        mutationRetryAvailable = cartMutationPending;
    ...
    function startSyncRun(requireChangedCart, explicitRetry = false) {
      ...
      return settleAndSync(
        ...
        explicitRetry && mutationRetryAvailable
      );
    }
    ...
    function usesApplePay() {
      return document.querySelector("#order_payment_service")?.value === "cyberbizpay_applepay";
    }

    function verifiedForDirectPayment() {
      const snapshot = currentDomCartSnapshot();
      return !checkoutBlocked && !cartMutationPending
        && verifiedRecordsStillCurrent()
        && lastVerifiedMutationVersion === cartMutationVersion
        && typeof lastVerifiedNoteValue === "string"
        && snapshot?.signature === lastVerifiedDomSignature;
    }

    document.addEventListener("click", event => {
      const checkoutButton = event.target.closest?.(CHECKOUT_BUTTON_SELECTOR);
      if (checkoutButton) {
        // ApplePaySession must be constructed in the original user gesture.
        // The final lifecycle/AJAX guards still enforce the verified note.
        if (usesApplePay() && verifiedForDirectPayment()) return;
        ...
        startSyncRun(false, true).then(synced => {
          ...
          if (synced && !checkoutBlocked && lastVerifiedMutationVersion === cartMutationVersion) {
            if (usesApplePay()) {
              renderPanel(lastVerifiedNoteValue + "\n\n資料已確認，請再次按下 Apple Pay 付款。", "");
              return;
            }
    ...
    if (typeof MutationObserver === "function") {
      const cartObserver = new MutationObserver(mutations => {
        if (!mutationsTouchCartControls(mutations)) return;
        const domSnapshot = currentDomCartSnapshot();
        if (!domSnapshot) return;
        // Checkout v3 may unmount/rebuild its cart rows for responsive or
        // sticky-layout changes (including ordinary scrolling). Those are not
        // cart edits. Ignore an unchanged signature, and ignore a transient
        // zero-row layout unless a quantity/delete action is already pending.
        if (
          !cartMutationPending
          && (
            domSnapshot.rowCount === 0
            || domSnapshot.signature === lastVerifiedDomSignature
          )
        ) return;
        notifyCartUpdate();
      });
    ...
    const boundCheckoutLibraries = new Set();
    function bindCheckoutLibrary(jq) {
      if (typeof jq !== "function" || boundCheckoutLibraries.has(jq)) return;
      boundCheckoutLibraries.add(jq);
      // Checkout v3 submits POST /carts/<token>/ through Helper.jQuery.ajax.
      // Guard the actual outgoing payload as well as the UI lifecycle event.
      // This never touches payment-token, customer, stock or cart-update APIs.
      if (typeof jq.ajaxPrefilter === "function") jq.ajaxPrefilter((options, original, xhr) => {
        let url;
        try { url = new URL(options.url, pageUrl.href); } catch (_) { return; }
        if (String(options.type || options.method || "GET").toUpperCase() !== "POST"
          || url.origin !== pageUrl.origin
          || url.pathname.replace(/\/$/, "") !== `/carts/${activeCartToken}`) return;
        const reject = () => {
          xhr.abort();
          blockUnverifiedDesigns("客製製作資料尚未確認，訂單未送出。請重新整理購物車後再試。");
        };
        if (checkoutBlocked || lastVerifiedMutationVersion !== cartMutationVersion
          || !verifiedRecordsStillCurrent()
          || typeof lastVerifiedNoteValue !== "string") return reject();
        // jQuery normally runs processData before its prefilters.
        if (typeof options.data === "string"
          && !/application\/json/i.test(options.contentType || "")) {
          const data = new URLSearchParams(options.data);
          data.set("order[note]", lastVerifiedNoteValue);
          options.data = data.toString();
        } else if (isPlainObject(options.data)) {
          options.data['order[note]'] = lastVerifiedNoteValue;
        } else return reject();
      });
      jq(document).on(
      ...
      jq(document).on(
        ...
        (event, payload) => {
          if (
            ...
            event?.preventDefault?.();
            event?.stopImmediatePropagation?.();
            // CYBERBIZ ignores preventDefault/return false from this event.
            // Its surrounding try/catch does stop on a string validation error.
            throw "客製製作資料尚未確認，訂單未送出。請重新整理購物車後再試。";
          }
    ...
    function bindCheckoutLibraries() {
      [window.jQuery, window.$, window.Helper?.jQuery].forEach(bindCheckoutLibrary);
    }
    bindCheckoutLibraries();
