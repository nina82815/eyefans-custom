/*
 * eYeFANS CYBERBIZ integration — UV COMBINED PRODUCT CANDIDATE V3.
 *
 * UNPUBLISHED PRODUCTION CANDIDATE V3 / DO NOT INSTALL ON A PUBLISHED THEME.
 *
 * Color and engraving still resolve a lens choice to one of three separate
 * CYBERBIZ products. UV now keeps all three lenses in one product, with an
 * exact SIZE × LENS variant for every combination. Seven target products and
 * all 36 variants are pinned below. This candidate may be installed only as a
 ...
 */
(function eyefansCyberbizCartUvCombinedProductionCandidateV3Loader() {
  "use strict";

  const TEST_QUERY_KEYS = Object.freeze(["eyefans_cart_test", "eyefans_cart_live_test"]);
  const UV_COMBINED_TEST_QUERY_KEY = "eyefans_uv_combined_live_test";
  ...
  const developmentRuntime = (() => {
    try {
      ...
      return loaderUrl.searchParams.get("eyefans_uv_combined_development") === "1";
    } catch (error) {
      return false;
    }
  })();
  ...
  // These retired UV variants must never be submitted again. They remain in
  ...
  if (!developmentRuntime && pageUrl.searchParams.get(UV_COMBINED_TEST_QUERY_KEY) === "1") return;
