/*
 * eYeFANS CYBERBIZ integration — guarded production cart bridge.
 *
...
(function eyefansCyberbizCartProductionLoader() {
  ...
  function publicErrorMessage(error) {
    const messages = {
      ...
      TOO_MANY_DESIGNS: "單次購物車的客製設計數量已達上限，請先完成本次結帳後再新增。"
    };
        ...
        addResult = await addVariantToCart(context, variantId, request.selection.size, {
          onPreflight(beforeQuantity, { canPost }) {
            ...
            const recordsWithoutStale = currentRecent
              ...
              : currentRecords;
            const candidateRecords = [...recordsWithoutStale, record];
