...
function createProductEnvironment({
  ...
  const document = {
    ...
    head: null,
    currentScript: {
      src: `https://example.invalid/cyberbiz-cart-production-loader-20260904-all-combined-v4.js${developmentRuntime ? "?eyefans_all_combined_development=1" : ""}`
    },
...
async function main() {
  assert.match(rawSource, /UNPUBLISHED PRODUCTION CANDIDATE V4 \/ DO NOT INSTALL ON A PUBLISHED THEME/);
  ...
  assert.ok(developmentEntrySource.includes(candidateSri),
    "development entry must pin the exact v4 candidate core SRI");
  ...
  for (const allowedHandle of Object.keys(PRODUCT_DEFINITIONS)) {
    assert.equal(
      ...
      1,
      `${allowedHandle} must load the v4 core on the explicit test URL`
    );
  }
  assert.equal(
    runLiveTestEntry(
    ...
    0,
    "the v4 wrapper must stay inert on the retired v3 test query"
  );
  ...
  assert.equal(
    ...
    "https://nina82815.github.io/eyefans-custom/integration/cyberbiz-cart-production-loader-20260904-all-combined-v4.js?eyefans_all_combined_development=1"
  );
  ...
  assert.equal(
    ...
    undefined,
    "the wrapper must not reserve the v4 core's own mutex before it loads"
  );
  ...
  console.log("cyberbiz cart all combined v4 tests passed");
}
