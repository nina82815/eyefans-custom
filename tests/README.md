...
2026-09-09 的 v4.2 在相同限定測試入口中，把手機底部原生購買列改成單一
「開始客製設計」動作，點擊只捲動到模擬器；手機立即購買與其他原生購買控制會被
攔截。安裝與回滾見 `integration/CYBERBIZ_CART_ALL_COMBINED_CANDIDATE_V4_2_20260909.md`。
...
node tests/cyberbiz-cart-all-combined-v4-2.test.js
node tests/cyberbiz-cart-mobile-cta-v4-2.test.js
node tests/cyberbiz-cart-note-sync-v4.test.js
node tests/cyberbiz-cart-note-sync-v4-1.test.js
node tests/cyberbiz-cart-note-sync-v4-2.test.js
