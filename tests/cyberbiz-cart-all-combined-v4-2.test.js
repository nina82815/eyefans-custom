...
const developmentEntryPath = path.join(
  ...
  "..",
  "integration",
  "cyberbiz-cart-all-combined-live-test-loader-20260909-v3.js"
);
...
function createProductEnvironment({
  ...
  const document = {
    ...
    head: null,
    currentScript: {
      src: `https://example.invalid/cyberbiz-cart-production-loader-20260909-all-combined-v4-2.js${loaderSearch}`
    },
...
function createCheckoutBoot(source, variantId, developmentRuntime = true) {
  ...
  const document = {
    ...
    documentElement: {},
    currentScript: {
      src: `https://example.invalid/cyberbiz-cart-production-loader-20260909-all-combined-v4-2.js${developmentRuntime ? "?eyefans_all_combined_development=1" : ""}`
    },
    scripts: [],
    addEventListener(type, listener) {
      documentListeners[type] = listener;
...
async function main() {
  assert.match(rawSource, /UNPUBLISHED PRODUCTION CANDIDATE V4\.2 \/ DO NOT INSTALL ON A PUBLISHED THEME/);
  assert.match(rawSource, /ProductionCandidateV4_2Loader/);
  ...
  assert.match(developmentEntrySource, /LiveTestEntryV3/);
  ...
  assert.ok(developmentEntrySource.includes(candidateSri),
    "development entry must pin the exact v4.2 candidate core SRI");
  ...
  for (const allowedHandle of Object.keys(PRODUCT_DEFINITIONS)) {
    assert.equal(
      ...
      1,
      `${allowedHandle} must load the v4.2 core on the explicit test URL`
    );
  }
  assert.equal(
    runLiveTestEntry(
    ...
    0,
    "the v4.2 wrapper must stay inert on the retired v3 test query"
  );
  ...
  assert.equal(
    ...
    "https://nina82815.github.io/eyefans-custom/integration/cyberbiz-cart-production-loader-20260909-all-combined-v4-2.js?eyefans_all_combined_development=1"
  );
  ...
  assert.equal(
    ...
    undefined,
    "the wrapper must not reserve the v4.2 core's own mutex before it loads"
  );
  assert.equal(explicitTestScripts.window.__eyefansCartAllCombinedLiveTestEntryV3Active, true,
    "the v4.2 wrapper must reserve its own mutex");
  ...
  assert.equal(
    ...
    "https://nina82815.github.io/eyefans-custom/integration/cyberbiz-cart-production-loader-20260909-all-combined-v4-2.js?eyefans_all_combined_development=1&eyefans_anniversary_preview=1",
  ...
  console.log("cyberbiz cart all combined v4.2 tests passed");
}
