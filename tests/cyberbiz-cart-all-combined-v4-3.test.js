...
const loaderPath = path.join(
  ...
  "..",
  "integration",
  "cyberbiz-cart-production-loader-20260910-all-combined-v4-3.js"
);
...
const developmentEntryPath = path.join(
  ...
  "..",
  "integration",
  "cyberbiz-cart-all-combined-live-test-loader-20260910-v4.js"
);
...
function createProductEnvironment({
  ...
  const document = {
    ...
    head: null,
    currentScript: {
      src: `https://example.invalid/cyberbiz-cart-production-loader-20260910-all-combined-v4-3.js${loaderSearch}`
    },
...
function createCheckoutBoot(source, variantId, developmentRuntime = true) {
  ...
  const document = {
    ...
    documentElement: {},
    currentScript: {
      src: `https://example.invalid/cyberbiz-cart-production-loader-20260910-all-combined-v4-3.js${developmentRuntime ? "?eyefans_all_combined_development=1" : ""}`
    },
    scripts: [],
    addEventListener(type, listener) {
      documentListeners[type] = listener;
...
function runLiveTestEntry(href, storedValue = null, initialFlags = {}) {
  ...
  const document = {
    currentScript: {
      src: "https://nina82815.github.io/eyefans-custom/integration/cyberbiz-cart-all-combined-live-test-loader-20260910-v4.js"
    },
    head: {
      appendChild(script) {
...
async function main() {
  assert.match(rawSource, /UNPUBLISHED PRODUCTION CANDIDATE V4\.3 \/ DO NOT INSTALL ON A PUBLISHED THEME/);
  assert.match(rawSource, /ProductionCandidateV4_3Loader/);
  ...
  assert.match(developmentEntrySource, /LiveTestEntryV4/);
  ...
  assert.ok(developmentEntrySource.includes(candidateSri),
    "development entry must pin the exact v4.3 candidate core SRI");
  ...
  for (const allowedHandle of Object.keys(PRODUCT_DEFINITIONS)) {
    assert.equal(
      ...
      1,
      `${allowedHandle} must load the v4.3 core on the explicit test URL`
    );
  }
  assert.equal(
    runLiveTestEntry(
    ...
    0,
    "the v4.3 wrapper must stay inert on the retired v3 test query"
  );
  ...
  assert.equal(
    ...
    "https://nina82815.github.io/eyefans-custom/integration/cyberbiz-cart-production-loader-20260910-all-combined-v4-3.js?eyefans_all_combined_development=1"
  );
  ...
  assert.equal(
    ...
    undefined,
    "the wrapper must not reserve the v4.3 core's own mutex before it loads"
  );
  assert.equal(explicitTestScripts.window.__eyefansCartAllCombinedLiveTestEntryV4Active, true,
    "the v4.3 wrapper must reserve its own mutex");
  ...
  assert.equal(
    ...
    "https://nina82815.github.io/eyefans-custom/integration/cyberbiz-cart-production-loader-20260910-all-combined-v4-3.js?eyefans_all_combined_development=1&eyefans_anniversary_preview=1",
  ...
  console.log("cyberbiz cart all combined v4.3 tests passed");
}
