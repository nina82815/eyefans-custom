  ...
  function cartRows() {
    ...
    if (previewGift || unknownExtraRow) {
      ...
      row.matches = selector => selector === "tr.line-item.extra.next-gift, div.line-item.extra.next-gift"
        ? previewGift : String(selector).includes("tr.line-item");
...
(async () => {
  ...
  const preview = createEnvironment(records, { previewGift: true });
  await flushMicrotasks();
  preview.runTimer(150);
  await flushMicrotasks();
  assert.equal(preview.checkout.attributes.has("aria-disabled"), false);
  assert.match(preview.note.value, /EF-TEST-POLAR1/);
  assert.match(preview.note.value, /EF-TEST-BLUE02/);
  assert.match(preview.note.value, /客人原有備註/);

  const unknownRow = createEnvironment(records, { unknownExtraRow: true });
  await flushMicrotasks();
  for (const delay of [150,350,750,1500]) {
    unknownRow.runTimer(delay);
    await flushMicrotasks();
  }
  assert.equal(unknownRow.checkout.attributes.get("aria-disabled"), "true");
  assert.equal(unknownRow.note.value, "客人原有備註");

  const earnedGift = createEnvironment(records, {
    extraItems: [cartItem("99999999", 1)],
    missingQuantityVariantId: "99999999"
  });
  await flushMicrotasks();
  earnedGift.runTimer(150);
  await flushMicrotasks();
  assert.equal(earnedGift.checkout.attributes.has("aria-disabled"), false);
  assert.match(earnedGift.note.value, /EF-TEST-POLAR1/);
